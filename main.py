...
async def create_order(call: CallbackQuery):
    ...
    text += f"""Umumiy buyurtmalar soni: {total_products}
To'lashingiz kerak bo'lgan summa: {total_price} 💸
Yetkazib berish summasi  12000 sum
"""
...
async def checkout(pre_checkout_query):
    await bot.answer_pre_checkout_query(pre_checkout_query.id,
                                        ok=True,
                                        error_message="Hisobingizda yeterli mablag' yo'q!")
...
async def send_location(message: types.Message):
    latitude = 41.26727128526837  # Широта (например, Москва)
    longitude = 69.20033527134302  # Долгота (например, Москва)
